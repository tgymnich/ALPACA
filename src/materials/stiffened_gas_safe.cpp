...
*                                                                                        *
*****************************************************************************************/
#include "stiffened_gas_safe.h"
#include "mathematical_functions.h"
#include <cmath>

/**
 * @brief Constructs a stiffened-gas limited object with material parameters given as input.
 * @param gamma .
 * @param A .
 * @param B .
 * @param C .
 ...
 */
StiffenedGasSafe::StiffenedGasSafe( double const gamma, double const B, double const mu_shear, double const mu_bulk ) :
   gamma_(gamma),
...
/**
 * @brief Computes pressure from inputs as -gamma*B + (gamma - 1) * (E  - 0.5 * rho * ||v^2||) (avoids devision by zero).
 * @param density .
 * @param momentum_x .
 * @param momentum_y .
 * @param momentum_z .
 ...
 */
double StiffenedGasSafe::DoGetEnthalpy( double const density, double const momentum_x, double const momentum_y, double const momentum_z, double const energy ) const {
   return (energy + DoGetPressure( density, momentum_x, momentum_y, momentum_z, energy )) / std::max( density, epsilon_ );
}

/**
 * @brief Computes energy from inputs as (p + gamma * B) / (gamma - 1) + 0.5 * rho * ||v^2|| (avoids devision by zero).
 * @param density .
 * @param momentum_x .
 * @param momentum_y .
 * @param momentum_z .
 ...
 * @return Gruneisen coefficent according to stiffened-gas equation of state.
 */
double StiffenedGasSafe::DoGetGruneisen() const {
   return (gamma_ - 1.0);
}
 ...
 */
double StiffenedGasSafe::DoGetPsi( double const pressure, double const one_density ) const {
 ...
 * @return gamma.
 */
double StiffenedGasSafe::DoGetGamma() const {
   return gamma_;
}
 ...
 * @return B.
 */
double StiffenedGasSafe::DoGetB() const {
   return background_pressure_;
}

/**
 * @brief Computes speed of sound from inputs as sqrt(gamma * (p + B)) / rho (avoids devision by zero).
 * @param density .
 * @param pressure .
 * @return Speed of sound according to stiffened-gas equation of state.
 */
double StiffenedGasSafe::DoGetSpeedOfSound( double const density, double const pressure ) const {
 ...
 */
std::vector<double> StiffenedGasSafe::DoGetViscosity() const {
